...
- Weekly refresh checked repositories changed in the last 7-10 days, prioritizing `kujo-skills`, the Publishing House workflow/tool repositories, `kujo-agents`, `kujo-workflows`, WebOps toolchain repositories, AI Chat, AI SDK, Watchdog, Dispatch, Relay, Workcell, Tribunal, and the v1-adjacent Kujo tool repositories.
- Publishing House records were updated for the current `kujo-skills` checkout at `b931c68`; the workflow lock now matches the sibling checkout used by the all-eleven fixture proof. The `kujo-publishing-house-workflows` skill itself was unchanged by the weekly skill drift commit.
- WebOps and general workflow skill routing records were preserved: `kujo-skills` added ContentGraph, Lens, SearchBridge, ShipCheck, SiteProbe, Kujo core, and standard-library guidance, but those changes were compatible expansions rather than workflow-catalog contract migrations.
- AI Chat still has dirty local experiment files, so only committed `479ce92` evidence was considered for catalog claims.
...
- [`WEEKLY_AUDIT_2026_08_15.md`](WEEKLY_AUDIT_2026_08_15.md) — latest weekly drift review, affected repositories, compatibility disposition, validation, and next watch list.
