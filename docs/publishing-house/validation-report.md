...
Captured on 2026-09-19 from the `main` branch. Generated run directories were
...
| `python3 scripts/validate_docs.py` | 0 | 184 Markdown files and 10 release files |
| `bash scripts/run-publishing-house-fixture.sh --out <temporary-directory>` | 0 | Covered through `python3 -m unittest discover -s tests -p 'test_*.py'`; initial pass exposed stale `kujo-agents` and `kujo-skills` support-lock commits, and the refreshed lock passed |
...
The 2026-09-19 refresh advanced the locked `kujo-agents` and `kujo-skills`
checkouts to the current clean commits used by the passing fixture proof. The
