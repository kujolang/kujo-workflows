...
Plan, validate, and package supported media assets with provenance, rights, accessibility, and exact checksums.

## Boundary

Unsupported transforms become bounded blockers; this workflow never publishes assets. Record ownership remains with assetworks, dossier, galleypack; this workflow stores references and orchestration receipts only.

## Run the deterministic fixture

```bash
(cd publishing-house-asset-production && bash bin/run --request fixtures/request.fixture.json --json)
```
...
Primary roles: Art Director, Video & Audio Producer, Production Editor. Required tools: assetworks, dossier, galleypack, dispatch, agents-sdk. Default permission: PROPOSE. Readiness: Limited technical preview; fixture evidence is verified locally, while live adapters and live-provider execution remain environment-specific.
